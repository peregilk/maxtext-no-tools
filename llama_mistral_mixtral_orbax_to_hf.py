...
def unpermute_from_match_maxtext_rope(arr, model_size):
  ...
  #if not (model_size[:8] == "llama3.1" or model_size[:8] == "llama3.x"):
  if model_size[:7] != "llama3.":
    print(model_size)
    print("####################")
...
def load_hf_model(model_size):
  ...
  if model_size == "llama2-7b":
    ...
    model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-3.1-70B",token="")
  elif model_size == "llama3.2-1b":
    model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-3.2-1B",token="")
  elif model_size == "llama3.2-3b":
    model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-3.2-3B",token="")
    config = AutoConfig.from_pretrained("meta-llama/Llama-3.2-3B")
  elif model_size == "llama3-8b":
    config = AutoConfig.from_pretrained("meta-llama/Meta-Llama-3-8B", token="")
...
def load_model_state(config):
  ...
  print("*******************")
  print(f"Decoder keys: {training_state.params['params']['decoder'].keys()}")
  print(f"Decoder norm keys: {training_state.params['params']['decoder']['decoder_norm'].keys()}")
  print(f"Decoder layer keys: {training_state.params['params']['decoder']['layers'].keys()}") 
