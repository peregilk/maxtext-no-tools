...
export RUN_NAME='llama3.1-8b-instruct'
...
export META_CHECKPOINT_PATH=gs://north-llama31-instruct
...
if [ -z "${BASE_OUTPUT_PATH}" ]; then
    ...
    # Use the same BASE_OUTPUT_PATH as end_to_end/tpu/llama3.1/8b/2_test_llama3.1_8b.sh
    export BASE_OUTPUT_PATH=gs://maxchkpt-eu/
...
JAX_PLATFORMS=cpu python3 MaxText/llama_or_mistral_ckpt.py --base-model-path /tmp/north-llama31-instruct --maxtext-model-path ${BASE_OUTPUT_PATH}/${RUN_NAME}/scanned_chkpt --model-size ${MODEL_VARIATION}
echo "Wrote MaxText compatible checkpoint to ${BASE_OUTPUT_PATH}/${RUN_NAME}/scanned_chkpt"
# We define `CONVERTED_CHECKPOINT` to refer to the checkpoint subdirectory.
export CONVERTED_CHECKPOINT=${BASE_OUTPUT_PATH}/${RUN_NAME}/scanned_chkpt/0/items
...
JAX_PLATFORMS=cpu python MaxText/generate_param_only_checkpoint.py MaxText/configs/base.yml async_checkpointing=false base_output_directory=${BASE_OUTPUT_PATH} load_parameters_path=${CONVERTED_CHECKPOINT} run_name=${RUN_NAME}/unscanned_checkpoint model_name=${MODEL_VARIATION} force_unroll=true
echo "Written MaxText compatible unscanned checkpoint to ${BASE_OUTPUT_PATH}/${RUN_NAME}/unscanned_checkpoint/checkpoints/0/items"
